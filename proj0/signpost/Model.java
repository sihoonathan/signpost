...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                int solSeqNum = _solution[x][y];
                if (solSeqNum == 1) {
                    _board[x][y] = new Sq(x, y, solSeqNum, true,
                            arrowDirection(x, y), 0);
                } else if (solSeqNum == last) {
                    _board[x][y] = new Sq(x, y, solSeqNum, true,
                            0, 0);
                } else {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                int solSeqNum = solution[x][y];
                _solnNumToPlace[solSeqNum] = _board[x][y].pl;
            }
        }
        for (int i = 1; i <= last; i++) {
            if (_solnNumToPlace[i] == null) {
                throw badArgs("doesn't have square assigned at i");
            }
        }
        for (Sq sq: _allSquares) {
            sq._successors = allSuccessors(sq.x, sq.y, sq._dir);
            for (Place pl : sq._successors) {
                if (_board[pl.x][pl.y]._predecessors == null) {
                    _board[pl.x][pl.y]._predecessors = new PlaceList();
                    _board[pl.x][pl.y]._predecessors.add(sq.pl);
                } else {
                    _board[pl.x][pl.y]._predecessors.add(sq.pl);
                }
            }
    ...
    Model(Model model) {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                _board[x][y]._successor = model._board[x][y]._successor;
                _board[x][y]._predecessor = model._board[x][y]._predecessor;
                _board[x][y]._head = model._board[x][y]._head;
            }
        }
    ...
    void init(int width, int height) {
        ...
        for (Sq[] col: _board) {
            for (Sq sq: col) {
                _allSquares.remove(sq);
            }
        }

        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                _allSuccessors[x][y] = null;
            }
        }
    ...
    boolean autoconnect() {
        int original = _unconnected;

        for (int i = 1; i <= _width * _height - 1; i++) {
            int x = _solnNumToPlace[i].x;
            int y = _solnNumToPlace[i].y;
            int x1 = _solnNumToPlace[i + 1].x;
            int y1 = _solnNumToPlace[i + 1].y;

            if (_board[x][y]._successor == null
                    && _board[x][y]._sequenceNum
                    == _board[x1][y1]._sequenceNum - 1) {
                _board[x][y].connect(_board[x1][y1]);
            }
        }

        boolean val = (original != _unconnected);
    ...
    void solve() {
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                ...
                if (solSeqNum == 1) {
                    continue;
                } else if (solSeqNum == _width * _height) {
                    continue;
                } else {
                    _board[x][y]._sequenceNum = solSeqNum;
                }
            }
        }
        autoconnect();
    ...
    private int arrowDirection(int x, int y) {
        ...
        for (int m = 0; m < _width; m++) {
            for (int n = 0; n < _height; n++) {
                if (seq1 == _solution[m][n]) {
                    int arrowDir = Place.dirOf(x, y, m, n);
                    return arrowDir;
                }
            }
        }
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            ...
            if (pl.dirOf(s1.pl) != 0 && s1.predecessor()
                    ==
                    null && successor() == null) {
                if (sequenceNum() == 0 && s1.sequenceNum() == 0) {
                    ...
                    return false;
                        ...
                        && s1.sequenceNum() == 0) || (sequenceNum() == 0
                        ) || (sequenceNum() == s1.sequenceNum() - 1)) {
                    return true;
                }
                return false;
        ...
        boolean connect(Sq s1) {
            ...
            int tgroup = this.group(); int sgroup = s1.group();
            _unconnected -= 1; _successor = s1;
            s1._predecessor = this;
            if (sequenceNum() != 0) {
                for (Sq sq = this; sq._successor != null;
                     sq = sq._successor) {
                    sq._successor._sequenceNum = sq._sequenceNum + 1;
                }
                releaseGroup(sgroup);
            ...
            if (s1.sequenceNum() != 0) {
                for (Sq sq = s1; sq._predecessor != null;
                     sq = sq._predecessor) {
                    sq._predecessor._sequenceNum = sq._sequenceNum - 1;
                }
                releaseGroup(tgroup);
            }
            Sq successorPtr = _successor;
            while (successorPtr != null) {
                successorPtr._head = this._head;
                successorPtr = successorPtr.successor();
            }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (this._predecessor == null && next._successor == null) {
                    ...
                    _head._group = -1; next._head._group = -1;
                ...
                boolean fixedPre = false;
                for (Sq sq = this; sq != null;
                    sq = sq._predecessor) {
                    if (sq.hasFixedNum()) {
                        fixedPre = true; break;
                    }
                ...
                if (!fixedPre) {
                    int possibleNew = newGroup();
                    for (Sq sq = this; sq != null;
                         sq = sq._predecessor) {
                        sq._sequenceNum = 0;
                        if (sq != null) {
                            sq._head._group = possibleNew;
                        } else {
                            sq._head._group = -1;
                        }
                    }
                }
                boolean fixedSuc = false;
                ...
                if (!fixedSuc) {
                    for (Sq sq = next; sq._successor != null;
                         sq = sq._successor) {
                        sq._sequenceNum = 0;
                    }
    ...
    private static final String[] ARROWS = {" *", "NE", "E ",
        "SE", "S ", "SW", "W ", "NW", "N "};
