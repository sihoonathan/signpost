...
class Place {
    ...
    static PlaceList[][][] successorCells(int width, int height) {
        ...
        PlaceList whole = new PlaceList();

        for (int m = 0; m < width; m++) {
            for (int n = 0; n < height; n++) {
                Place temp = new Place(m, n);
                whole.add(temp);
            }
        }
        ...
        for (Place sq1 : whole) {
            for (Place sq2 : whole) {
                int direction = sq1.dirOf(sq2);
                if (direction != 0) {
                    PlaceList pLDir = M[sq1.x][sq1.y][direction];
                    PlaceList pLZero =  M[sq1.x][sq1.y][0];

                    if (pLDir == null) {
                        M[sq1.x][sq1.y][direction] = new PlaceList();
                        M[sq1.x][sq1.y][direction].add(sq2);
                    } else {
                        pLDir.add(sq2);
                    }

                    if (pLZero == null) {
                        M[sq1.x][sq1.y][0] = new PlaceList();
                        M[sq1.x][sq1.y][0].add(sq2);
                    } else {
                        pLZero.add(sq2);
                    }
                }
