...
class PuzzleGenerator implements PuzzleSource {
    ...
    @Override
    public Model getPuzzle(int width, int height,
                           boolean allowFreeEnds) {
        Model model = new Model(makePuzzleSolution
                (width, height, allowFreeEnds));
    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                for (Place sq1: sq.successors()) {
                    if (sq.connectable(model.get(sq1))) {
                        nFound += 1;
                        found = model.get(sq1);
                    }
                }
                ...
                if (sq.sequenceNum() != 0) {
                    for (Place sq1: sq.successors()) {
                        if (model.get(sq1).sequenceNum() != 0
                                && sq.connectable(model.get(sq1))) {
                            nFound = 1;
                            found = model.get(sq1);
                        }
                    }
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                for (Place sq1: sq.predecessors()) {
                    if (model.get(sq1).connectable(sq)) {
                        nFound += 1;
                        found = model.get(sq1);
                    }
                }
